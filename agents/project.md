...
- `lib/` — gitignored per-developer drop folder for reference assemblies the game install lacks (MelonLoader on a UMM-only box, and vice versa). The install always wins; `lib/` is a fallback search path only, and `Assembly-CSharp*` is never taken from it.
